...
void DGAcoustics3D::allocateScratch() {
    ...
    dpdr_.resize(Np, K); dpds_.resize(Np, K); dpdt_.resize(Np, K);
    duxdr_.resize(Np, K); duxds_.resize(Np, K); duxdt_.resize(Np, K);
    duydr_.resize(Np, K); duyds_.resize(Np, K); duydt_.resize(Np, K);
    duzdr_.resize(Np, K); duzds_.resize(Np, K); duzdt_.resize(Np, K);
...
int DGAcoustics3D::findElement(const Vec3d& pos) const {
    ...
    for (int k = 0; k < mesh_.K; ++k) {
        ...
        cx /= basis_.Np; cy /= basis_.Np; cz /= basis_.Np;
        double d = (pos.x()-cx)*(pos.x()-cx) + (pos.y()-cy)*(pos.y()-cy) + (pos.z()-cz)*(pos.z()-cz);
        if (d < bestDist) { bestDist = d; bestElem = k; }
    }
    return bestElem;
}
        ...
        for (int f = 0; f < Nfaces; ++f) {
            ...
            for (int i = 0; i < Nfp; ++i) {
                ...
                if (isBoundary) {
                    ...
                    pP  = p(nP, kP);
                    uxP = ux(nP, kP); uyP = uy(nP, kP); uzP = uz(nP, kP);
                }
...
AcousticsResult3D DGAcoustics3D::solve(
    ...
    CancelCheck cancel)
{
    ...
    sourceWeights_.resize(Np);
    {
        ...
        if (totalW > 1e-30) sourceWeights_ /= totalW;
    }
    ...
    if (progress) {
        ...
        progress(0, "DG 3D: " + std::to_string(K) + " elems, "
    ...
    for (int step = 0; step < nSteps; ++step) {
        ...
        if (progress && step >= warmupSteps) {
            ...
            if (step % reportEvery == 0) {
                ...
                std::string msg = "DG 3D: step " + std::to_string(step)
    ...
    if (progress) {
        ...
        progress(100, "DG 3D: done in " + std::to_string(static_cast<int>(total)) + "s ("
